package sofa.v4_test

import data.sofa.v4 as sofa
import rego.v1

# -- fixture: minimal valid SoFA --
_valid := {
...
test_valid_input if {
	sofa.valid with input as _valid with data.schema as _schema with data.thresholds as _thresholds
}

# 2. Missing required field triggers error
test_missing_required_field if {
	bad := object.remove(_valid, ["entity_name"])
	some d in sofa.deny with input as bad with data.schema as _schema with data.thresholds as _thresholds
	d.field == "entity_name"
	d.severity == "error"
}
